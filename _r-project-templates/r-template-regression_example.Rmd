---
title: "Regression Model for [Project Name] Using R version 10"
author: "David Lowe"
date: "June 3, 2019"
output: 
  html_document: 
...
SUMMARY: [The purpose of this project is to construct a prediction model using various machine learning algorithms and to document the end-to-end steps using a template. The [Dataset Name] is a regression situation where we are trying to predict a real-number value for a given set of criteria.]
...
ANALYSIS: [Sample Paragraph - The baseline performance of the machine learning algorithms achieved an average RMSE of 4.561. Two algorithms (Random Forest and Gradient Boosting) achieved the top RMSE metrics after the first round of modeling. After a series of tuning trials, Random Forest turned in the top overall result and achieved an RMSE metric of 3.105. By using the optimized parameters, the Random Forest algorithm processed the testing dataset with a RMSE of 3.480, which was slightly above the prediction RMSE gained from the training data.]

CONCLUSION: [Sample Paragraph - For this iteration, the Random Forest algorithm achieved the best overall training and validation results. For this dataset, the Random Forest algorithm could be considered for further modeling or production use.]
...
```{r RIDGE}
# Ridge (Regression)
email_notify(paste("Ridge Regression modeling has begun!",date()))
startTimeModule <- proc.time()
set.seed(seedNum)
fit.ridge <- train(targetVar~., data=xy_train, method="ridge", metric=metricTarget, trControl=control)
print(fit.ridge)
proc.time()-startTimeModule
email_notify(paste("Ridge Regression modeling completed!",date()))
```

```{r LASSO}
# The Lasso (Regression)
email_notify(paste("The Lasso modeling has begun!",date()))
startTimeModule <- proc.time()
set.seed(seedNum)
fit.lasso <- train(targetVar~., data=xy_train, method="lasso", metric=metricTarget, trControl=control)
print(fit.lasso)
proc.time()-startTimeModule
email_notify(paste("The Lasso modeling completed!",date()))
```

```{r ENET}
# Elasticnet (Regression)
email_notify(paste("Elasticnet modeling has begun!",date()))
startTimeModule <- proc.time()
set.seed(seedNum)
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(mtry = c(4,7,10,13))
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(nrounds=c(50,100,200,300), max_depth=3, eta=0.3, gamma=0, colsample_bytree=0.8, min_child_weight=1, subsample=0.75)
...
finalModel <- randomForest(targetVar~., xy_complete, mtry=13, na.action=na.omit)
summary(finalModel)
proc.time()-startTimeModule
```
